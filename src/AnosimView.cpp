// AnosimView.cpp : implementation file
//
...
IMPLEMENT_DYNCREATE(CAnosimView, CViewExtension)

CAnosimView::CAnosimView()
{
	InitializeFonts();
}
...
CAnosimView::~CAnosimView()
{
	if(m_columns && !m_pDoc->m_LoadingAnosimCol){
		m_pDoc->mr_anosimcol.bottom = -1;
	}else if(!m_pDoc->m_LoadingAnosimRow){
		m_pDoc->mr_anosimrow.bottom = -1;
	}
}


BEGIN_MESSAGE_MAP(CAnosimView, CViewExtension)
	//{{AFX_MSG_MAP(CAnosimView)
	ON_WM_CREATE()
	ON_WM_DESTROY()
	ON_WM_HSCROLL()
	ON_WM_VSCROLL()
	...
	ON_WM_CONTEXTMENU()
	ON_COMMAND(ID__EXPORT, OnSaveas)
	ON_WM_MOUSEWHEEL()
	ON_COMMAND(ID__BEST, OnBest)
	ON_COMMAND(ID__STAT, OnStat)
	ON_COMMAND(ID__SAVE, OnSave)
...
/////////////////////////////////////////////////////////////////////////////
// CAnosimView diagnostics

#ifdef _DEBUG
void CAnosimView::AssertValid() const
{
	CView::AssertValid();
}

void CAnosimView::Dump(CDumpContext& dc) const
{
	CView::Dump(dc);
}
#endif //_DEBUG
...
/////////////////////////////////////////////////////////////////////////////
// CAnosimView message handlers



int CAnosimView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	m_pDoc = pDoc;
	m_columns = (m_pDoc->m_AnosimPairView)?true:false;	

	m_menu.LoadMenu(IDR_TABLEANOSIM);
	...
	if(m_columns){
		m_pMatrix = &m_pDoc->m_ColPairStat;
		grpidxs = &m_pDoc->m_AnosimColGroups;
	}else{
		m_pMatrix = &m_pDoc->m_RowPairStat;
	...
	for(i=0;i<grpidxs->GetSize();i++){
		...
		for(j=0;j<m_GrpIdxs.GetSize();j++){
			if(grp == m_GrpIdxs[j]){
				j = -1;
				break;
			}else if(grp < m_GrpIdxs[j]){
				m_GrpIdxs.InsertAt(j,grp);
				j = -1;
				break;
			}					
		}
	...
	SetScrollRange(SB_VERT,0,m_pMatrix->GetSize()-1,TRUE);
	SetScrollRange(SB_HORZ,0,m_pMatrix->GetSize()-1,TRUE);
...
void CAnosimView::OnDestroy() 
{
	CView::OnDestroy();
	
	if(!m_columns)
		m_pDoc->m_AnosimRowView	= false;
	else
		m_pDoc->m_AnosimColView = false;
}

void CAnosimView::OnDraw(CDC* pDC)
{
	pDC->SetTextColor(m_pDoc->m_rgbFontCells);
	if(m_columns){
		if(!m_pDoc->m_ANOSIMColsExists){
			pDC->TextOut(20,20,"Column Anosim needs to be calculated again.");
			return;
		}
	}else{
		if(!m_pDoc->m_ANOSIMRowsExists){
			pDC->TextOut(20,20,"Row Anosim needs to be calculated again.");
			return;
		}
	}
    ...
    //if ( ! fontData.CreateFontIndirect(&m_lfData) )
	//{
	//	fontHeading.DeleteObject();
	...
	//set title - check for stat/save/best
	if(m_columns){
		if(m_pMatrix == &m_pDoc->m_ColPairStat)
			sprintf(buffer,"Column Anosim with Group Pairs (%% Randomised > Real)");
		else if(m_pMatrix == &m_pDoc->m_ColPairBest)
			sprintf(buffer,"Column Anosim with Group Pairs (Best F)");
		else
			sprintf(buffer,"Column Anosim with Group Pairs (Real F)");
	}else{
		if(m_pMatrix == &m_pDoc->m_RowPairStat)
			sprintf(buffer,"Row Anosim with Group Pairs (%% Randomised > Real)");
		else if(m_pMatrix == &m_pDoc->m_RowPairBest)
			sprintf(buffer,"Row Anosim with Group Pairs (Best F)");
		else
			sprintf(buffer,"Row Anosim with Group Pairs (Real F)");			 
	}
	...
	pDC->ExtTextOut(x,y,NULL,rText,buffer,strlen(buffer),NULL);
	// ========================================================================
	...
	if ( y > rClip.top && y < rClip.bottom )
	{
		for ( j = m_idxLeftColumn; j < nRows; j++ )
		{
			...
			if(j < m_GrpIdxs.GetSize() && j >= 0)
				strLabel.Format("Group %d",m_GrpIdxs[j]);
	...
	for ( ; i < nRows; i++ )
	{
		...
		rText.right = CXCOLUMNWIDTH;
		x = rText.right - tm.tmAveCharWidth/2;
		if(i+1 >= 0 && i+1 < m_GrpIdxs.GetSize())
			strLabel.Format("Group %d",m_GrpIdxs[i+1]);
		...
		pAMR = NULL;
		if(m_pMatrix != NULL && i<m_pMatrix->GetSize())
			pAMR = (CAssocMatrixRow*)m_pMatrix->GetAt(i);
		if(pAMR != NULL)
			nCols = pAMR->GetSize();
		if ( pDC->IsPrinting() )
		...
		for ( ; j < nCols; j++ )
		{
			if(pAMR != NULL && j < pAMR->GetSize())
				nValue = pAMR->GetAt(j);
			if(nValue != m_pDoc->GetMissingValue())
...
void CAnosimView::InitializeFonts()
{
	m_nHeadingPointSize = 16;
	m_nHeaderFooterPointSize = 10;

	m_lfHeading.lfHeight = 10; // to be set when device context is prepared
	...
	m_lfHeaderFooter.lfHeight = 10; // to be set when device context is prepared
...
BOOL CAnosimView::CalcMetrics(CDC* pDC)
{
	CFont font;
	TEXTMETRIC tm;
...
void CAnosimView::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
	...
	switch(nSBCode)
	{
		...
		case SB_LINERIGHT:
			if ( m_idxLeftColumn < m_pMatrix->GetSize()-1 )
			{
				m_idxLeftColumn++;
				nScrollAmount = -CXCOLUMNWIDTH;
			}
		...
		case SB_PAGERIGHT:
			while ( m_idxLeftColumn < m_pMatrix->GetSize()-1 )
			{
				m_idxLeftColumn++;
				nScrollAmount += CXCOLUMNWIDTH;
...
void CAnosimView::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
	...
	switch(nSBCode)
	{
		...
		case SB_LINEDOWN:
			if ( m_idxTopRow < m_pMatrix->GetSize()-1 )
			{
				m_idxTopRow++;
				nScrollAmount = -CYLINESPACING;
			}
		...
		case SB_PAGEDOWN:
			while ( m_idxTopRow < m_pMatrix->GetSize()-1 )
			{
				m_idxTopRow++;
				nScrollAmount += CYLINESPACING;
...
void CAnosimView::OnPrepareDC(CDC* pDC, CPrintInfo* pInfo) 
{
...
void CAnosimView::SetMapping(CDC* pDC)
{
...
BOOL CAnosimView::DestroyWindow() 
{
	return CView::DestroyWindow();
}

void CAnosimView::OnBeginPrinting(CDC* pDC, CPrintInfo* pInfo) 
{
//	CRect rect,temp;
//	CPoint point;
//
...
//	m_rectDraw.bottom = (long)((point.y - rect.bottom)/((double) point.y)*m_VRes);
	...
	m_AssocMxMetrics.cyLineSpacing *= (long)(3);//*m_VScale);
	m_AssocMxMetrics.cxColumnWidth *= (long)(2);//*m_HScale);
	m_tempheight = m_lfData.lfHeight;
	m_pDoc->m_lfCells.lfHeight = (long) (m_tempheight*m_HScale*3);	//if(m_psd != NULL) delete m_psd;	m_psd = NULL;
	...
	idx = -1;
	startidx = idx;
	double ypos = CYLINESPACING*5*m_VScale; //header info space
	while(startidx < count - 1){
	...
	m_pDoc->m_lfCells.lfHeight = m_tempheight;
	if(m_pDoc->m_lfCells.lfHeight > 0) m_pDoc->m_lfCells.lfHeight *= -1;
	m_AssocMxMetrics.cyLineSpacing /= (long)(3);//*m_VScale);
	m_AssocMxMetrics.cxColumnWidth /= (long)(2);//*m_HScale);
}
...
BOOL CAnosimView::OnPreparePrinting(CPrintInfo* pInfo)
{
	m_pageinfo.SetSize(0);
...
void CAnosimView::OnEndPrinting(CDC* /*pDC*/, CPrintInfo* /*pInfo*/)
{
	long i;
...
void CAnosimView::OnPrint(CDC* pDC, CPrintInfo* pInfo) 
{
	...
	long tempheight = m_pDoc->m_lfCells.lfHeight;
	m_pDoc->m_lfCells.lfHeight = (long) (m_pDoc->m_lfCells.lfHeight*m_HScale*3);
	if(m_pDoc->m_lfCells.lfHeight >0) m_pDoc->m_lfCells.lfHeight *=-1;
	fontData.CreateFontIndirect(&m_pDoc->m_lfCells);
	m_pDoc->m_lfCells.lfHeight = tempheight;
	if(m_pDoc->m_lfCells.lfHeight >0) m_pDoc->m_lfCells.lfHeight *=-1;
	...
	rect.right += (long)(CYLINESPACING*m_VScale);
	...
	for(i=m_pageinfo.GetAt(cPage)->left;i<=m_pageinfo.GetAt(cPage)->right;i++){
		if(i == -1){
			...
			if(cPage == 0){
				...
				//draw header here
				text.Format("Data Table: %s",m_pDoc->m_TableName);
				...
				if(m_columns){
					if(m_pMatrix == &m_pDoc->m_ColPairStat)
						text = "Column Anosim with Group Pairs (Stat)";
					else if(m_pMatrix == &m_pDoc->m_ColPairBest)
						text = "Column Anosim with Group Pairs (Best)";
					else
						text = "Column Anosim with Group Pairs (Save)";
				}else{
					if(m_pMatrix == &m_pDoc->m_RowPairStat)
						text = "Row Anosim with Group Pairs (Stat)";
					else if(m_pMatrix == &m_pDoc->m_RowPairBest)
						text = "Row Anosim with Group Pairs (Best)";
					else
						text = "Row Anosim with Group Pairs (Save)";			 
				}
				...
				pDC->ExtTextOut(rect.left,rect.bottom, ETO_CLIPPED, 
					rect,text,text.GetLength(),NULL);
				rect.top = rect.bottom;//+(long)(CYLINESPACING*2*m_VScale);
				rect.bottom += (long)(CYLINESPACING*m_VScale);	
			}else{
				rect.top = m_rectDraw.top+(long)(CYLINESPACING*3*m_VScale);
				rect.bottom = rect.top + (long)(CYLINESPACING*m_VScale);
			}
			rect.left = rect.right = m_rectDraw.left;
		...
		for(j=m_pageinfo.GetAt(cPage)->top;j<=m_pageinfo.GetAt(cPage)->bottom;j++){
			if(i == -1 && j != -1){
				...
				//rect.right += (long)(CXCOLUMNWIDTH*m_HScale);
				//printrow title
				if(j+1 >= 0 && j < m_GrpIdxs.GetSize())
					text.Format("Group %d",m_GrpIdxs[j+1]);
				...
				//printcol title
				if( i>=0 && i < m_GrpIdxs.GetSize())
					text.Format("Group %d",m_GrpIdxs[i]);
				...
				//rect.left -= (long)(CXCOLUMNWIDTH*m_HScale);
				...
				if(j >= i) {
					...
					if(m_pMatrix != NULL && j>=0 && i>=0 &&
						j < m_pMatrix->GetSize() && 
						m_pMatrix->GetAt(j) != NULL &&
						i < m_pMatrix->GetAt(j)->GetSize()){
						if(m_pMatrix->GetAt(j)->GetAt(i) == m_pDoc->GetMissingValue())
							text = "invalid";
						else
							text.Format("%.4f",m_pMatrix->GetAt(j)->GetAt(i));
					}
...
void CAnosimView::OnOptionsFont()
{
...
void CAnosimView::OnContextMenu(CWnd* pWnd, CPoint point)
{
...
BOOL CAnosimView::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt) 
{
...
BOOL CAnosimView::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName, DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID, CCreateContext* pContext) 
{
...
void CAnosimView::OnBest() 
{
	m_menu.CheckMenuItem(ID__BEST,MF_CHECKED);
	m_menu.CheckMenuItem(ID__STAT,MF_UNCHECKED);
	m_menu.CheckMenuItem(ID__SAVE,MF_UNCHECKED);

	if(m_columns)
		m_pMatrix = &m_pDoc->m_ColPairBest;
	else
		m_pMatrix = &m_pDoc->m_RowPairBest;	

	RedrawWindow();
}

void CAnosimView::OnStat() 
{
	m_menu.CheckMenuItem(ID__BEST,MF_UNCHECKED);
	m_menu.CheckMenuItem(ID__STAT,MF_CHECKED);
	m_menu.CheckMenuItem(ID__SAVE,MF_UNCHECKED);
...
void CAnosimView::OnSave() 
{
	m_menu.CheckMenuItem(ID__BEST,MF_UNCHECKED);
	m_menu.CheckMenuItem(ID__STAT,MF_UNCHECKED);
	m_menu.CheckMenuItem(ID__SAVE,MF_CHECKED);

	if(m_columns)
		m_pMatrix = &m_pDoc->m_ColPairSave;
	else
		m_pMatrix = &m_pDoc->m_RowPairSave;	

	RedrawWindow();
}

void CAnosimView::OnClose() 
{
