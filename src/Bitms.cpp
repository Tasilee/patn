// Bitms.cpp: implementation of the CBitms class.
//
//////////////////////////////////////////////////////////////////////
...
LoadDIBitmap(const char *filename, /* I - File to load */
    ...
    long              infosize;     /* Size of header information */
    ...
    if ((long) fread(*info, 1, infosize, fp) < infosize)
        {
    ...
    if ((long)fread(bits, 1, bitsize, fp) < bitsize)
        {
...
SaveDIBitmap(const char *filename, /* I - File to load */
    ...
    if ((long)fwrite(info, 1, infosize, fp) < infosize)
        {
    ...
    if ((long)fwrite(bits, 1, bitsize, fp) < bitsize)
        {
...
static unsigned long   read_dword(FILE *fp);
static long            read_long(FILE *fp);

static long            write_word(FILE *fp, unsigned short w);
static long            write_dword(FILE *fp, unsigned long dw);
static long            write_long(FILE *fp, long l);
...
LoadDIBitmap(const char *filename, /* I - File to load */
    ...
    long              x, y;         /* X and Y position in image */
...
static unsigned long               /* O - 32-bit unsigned integer */
...
static long                        /* O - 32-bit signed integer */
...
static long                     /* O - 0 on success, -1 on error */
...
write_dword(FILE         *fp, /* I - File to write to */
            unsigned long dw)  /* I - Integer to write */
    {
    putc(dw, fp);
    putc(dw >> 8, fp);
    putc(dw >> 16, fp);
