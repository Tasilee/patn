// ConstFidView.cpp : implementation file
//
...
IMPLEMENT_DYNCREATE(CConstFidView, CView)

CConstFidView::CConstFidView()
{
	//fill data
	InitializeFonts();
}
...
BEGIN_MESSAGE_MAP(CConstFidView, CView)
	ON_WM_PAINT()
	ON_WM_VSCROLL()
	ON_WM_HSCROLL()
	ON_WM_CREATE()
...
// CConstFidView drawing

void CConstFidView::OnDraw(CDC* pDC)
{
	SetScrollRange(SB_VERT,0,m_rowLabels->GetSize()-1,TRUE);
	SetScrollRange(SB_HORZ,0,m_colLabels->GetSize()-1,TRUE);
...
/*	if(pDoc->m_AssocRowChange &&
		...
		*/
	sprintf(buffer,"Threashold for display of Constancy value: %.0f with %s Groups",m_threashold,m_isApriori?"Apriori":"PATN");
...
*/
	...
	{
		for ( j = m_idxLeftColumn; j < m_colLabels->GetSize(); j++ )
		{
			rText.left = rText.right;
			rText.right += CXCOLUMNWIDTH;
			x = rText.right - tm.tmAveCharWidth/2;

			strLabel = m_colLabels->GetAt(j);
	...
	for ( ; i < nRows; i++ )
	{
		...
		rText.right = this->m_AssocMxMetrics.cx1stColumnWidth;
		x = rText.right - tm.tmAveCharWidth/2;
		strLabel = m_rowLabels->GetAt(i);

		pDC->ExtTextOut(x,y,ETO_CLIPPED | ETO_OPAQUE,rText,strLabel,strLabel.GetLength(),NULL);

		// data
		pDC->SelectObject(&fontData);
		pAMR = (dblarray*)m_data->GetAt(i);
		nCols = pAMR->GetSize();
		if ( pDC->IsPrinting() )
		...
		{
			nValue = pAMR->GetAt(j);
			if(i == 1 || (nValue < m_threashold && i > 1)) {
				sprintf(buffer,"");
			} else {
				sprintf(buffer,"%0.0f",nValue);
			}
...
// CConstFidView diagnostics

#ifdef _DEBUG
void CConstFidView::AssertValid() const
{
	CView::AssertValid();
}

void CConstFidView::Dump(CDumpContext& dc) const
{
	CView::Dump(dc);
}
#endif //_DEBUG
...
void CConstFidView::InitializeFonts()
{
...
BOOL CConstFidView::CalcMetrics(CDC* pDC)
{
	CFont font;
	TEXTMETRIC tm;
	...
	if ( pDC->IsPrinting() )
		...
		i = m_idxTopRow;
	int top = 0;
	CPatnDoc * pDoc = (CPatnDoc*) GetDocument();CString strLabel;
...
void CConstFidView::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	switch(nSBCode)
	{
		...
		case SB_LINEDOWN:
			if ( m_idxTopRow < m_data->GetSize()-1 )
			{
				m_idxTopRow++;
				nScrollAmount = -CYLINESPACING;
			}
		...
		case SB_PAGEDOWN:
			while ( m_idxTopRow < m_data->GetSize()-1 )
			{
				m_idxTopRow++;
				nScrollAmount += CYLINESPACING;
...
void CConstFidView::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	if(nSBCode == SB_THUMBTRACK || nSBCode == SB_THUMBPOSITION)
    {
    // First determine if the user scrolled a scroll bar control
	...
	switch(nSBCode)
	{
		...
		case SB_LINERIGHT:
			if ( m_idxLeftColumn < m_colLabels->GetSize() - 1 )
			{
				m_idxLeftColumn++;
				nScrollAmount = -CXCOLUMNWIDTH;
			}
		...
		case SB_PAGERIGHT:
			while ( m_idxLeftColumn < m_colLabels->GetSize()-1 )
			{
				m_idxLeftColumn++;
				nScrollAmount += CXCOLUMNWIDTH;
...
int CConstFidView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	m_psd = NULL;

	m_data = NULL;

	if (CViewExtension::OnCreate(lpCreateStruct) == -1)
		return -1;
...
void CConstFidView::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
...
delete pFileDlg;
	...
	{
		strPathName = ((CPatnApp*)AfxGetApp())->sm_data + " Constancy " + (!m_isApriori?"PATN generated groups":"Apriori groups") + ".csv";
	}
	...
	f.WriteString(tmp);
	if(m_isApriori) {
		f.WriteString("aPriori Groups");
	} else {
		f.WriteString("PATN Generated Groups");
	}
	tmp.Format("\nThreashold for display of Constancy value: %.0f",m_threashold);
	f.WriteString(tmp);
	f.WriteString("\n\n\n");
	...
	for(j=-1;j<m_data->GetSize();j++){
		for(i=-1;i<m_data->GetAt(0)->GetSize();i++) {
			if(i == -1 && j != -1){
				//printrow title
				f.WriteString("\n");
				f.WriteString(m_rowLabels->GetAt(j));
			}else if(j==-1 && i == -1) {
			}else if(j == -1 && i != -1 && i < m_colLabels->GetSize()){ //-1 && i != -1){
				//printcol title
				f.WriteString(",");
				f.WriteString(m_colLabels->GetAt(i));
			}else if(j != -1 && i != -1){
				//print cell contents
				if(j == 1 || (m_data->GetAt(j)->GetAt(i) < m_threashold && j > 1)) {
					str = ",";
				} else {
					str.Format(",%.6f",m_data->GetAt(j)->GetAt(i));
				}
				f.WriteString(str);
			}
		}
...
BOOL CConstFidView::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	...
	long max = m_rowLabels->GetSize()-1;
...
BOOL CConstFidView::DestroyWindow()
{
...
BOOL CConstFidView::PreCreateWindow(CREATESTRUCT& cs)
{
	// TODO: Add your specialized code here and/or call the base class
...
void CConstFidView::OnBeginPrinting(CDC* pDC, CPrintInfo* pInfo)
{
	CPatnDoc* pDoc = (CPatnDoc*) GetDocument();
	ASSERT_VALID(pDoc);
...
//	m_rectDraw.bottom = (long)((point.y - rect.bottom)/((double) point.y)*m_VRes);
	...
	long count = m_rowLabels->GetSize();
...
BOOL CConstFidView::OnPreparePrinting(CPrintInfo* pInfo)
{
m_pageinfo.SetSize(0);
...
void CConstFidView::OnPrepareDC(CDC* pDC, CPrintInfo* pInfo)
{
...
BOOL CConstFidView::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName, DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID, CCreateContext* pContext)
{
...
void CConstFidView::OnEndPrinting(CDC* pDC, CPrintInfo* pInfo)
{
long i;
...
void CConstFidView::OnPrint(CDC* pDC, CPrintInfo* pInfo)
{
CPatnDoc* pDoc = (CPatnDoc*) GetDocument();
	ASSERT_VALID(pDoc);
	...
	for(i=m_pageinfo.GetAt(cPage)->left;i<=m_pageinfo.GetAt(cPage)->right;i++){
		if(i == -1){
			...
			/*	if(m_pMatrix == &pDoc->m_AssocMatrixRows){
				...
				text.Format("Constancy Threashold for display: %.0f",m_threashold);
				pDC->ExtTextOut(rect.left,rect.bottom, ETO_CLIPPED, 
					rect,text,text.GetLength(),NULL);
				rect.top = rect.bottom+1;
				rect.bottom += (long)(CYLINESPACING*m_VScale);
	...
	*/
		...
		for(j=m_pageinfo.GetAt(cPage)->top;j<=m_pageinfo.GetAt(cPage)->bottom;j++){
			if(i == -1 && j != -1){
				...
				if(j < m_data->GetSize() && i < m_data->GetAt(j)->GetSize()) {
					//text.Format("%.2f",pDoc->GetValueAt(i,j));
					if(j == 1 || (j > 1 && m_data->GetAt(j)->GetAt(i) < m_threashold)) {
						text = "";
					} else {
						text.Format("%.0f",m_data->GetAt(j)->GetAt(i));
					}
...
void CConstFidView::OnOptionsFont() 
{
